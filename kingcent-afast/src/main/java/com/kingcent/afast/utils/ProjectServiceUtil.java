...
import static com.kingcent.afast.utils.ProjectDaoUtil.daoIsMysqlSource;
...
public class ProjectServiceUtil {
    ...
    public static String generateJava(
            ...
            ProjectDaoEntity daoEntity,
            ProjectServiceEntity service,
            List<ProjectServiceFuncEntity> functions,
            boolean useMybatis,
        ...
        //类名
        String name = formatServiceName(service.getName());
        //类注解
        ...
        //处理mysql数据源类型的dao
        if(daoEntity != null && daoIsMysqlSource(daoEntity)){
            if (useMyBatisPlus){
                extendsStr = " extends IService<"+entityName+">";
                imports.add("import com.baomidou.mybatisplus.extension.service.IService;");
        ...
        if(functions != null && functions.size() > 0) {
            for (ProjectServiceFuncEntity function : functions) {
